...
def update_cognitive_ledger(supabase_client, user_id, recent_chat_history):
    ...
    try:
        ...
        state_response = supabase_client.table("user_cognitive_state").select("interest_matrix").eq("user_id", user_id).execute()
        current_matrix = state_response.data[0].get("interest_matrix", {}) if state_response.data else {}

        # 2. The Multi-Domain System Prompt
        ...
        # 3. Save the mutated brain
        supabase_client.table("user_cognitive_state").upsert({
            "user_id": user_id,
